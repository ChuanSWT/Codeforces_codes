...
void compete_solution()
{
    ...
    int n,m;
    cin>>n>>m;
    matrix raw_persons=cin_edges(2,n+m+1);
    matrix persons;

    for(int i=0;i<n+m+1;++i){
        persons.push_back({raw_persons[0][i],raw_persons[1][i]});
    }
    //for(auto arr:persons)
    //    print(arr);
    int start=cal_start(n,m,persons);
    int tot=cal_tot(n,m,persons);
    //取消一个n 取消一个m
    line other={cal_n(n,m,persons),cal_m(n,m,persons)};
    //cout<<tot<<endl;
    //print(other);
    for(int i=0;i<n+m+1;++i){
        int nv=persons[i][0];
        ...
        if(i>=start){
            if(flag)
                ans.push_back(tot-mv);
            else
                ans.push_back(tot-nv);
        }
        else{
            if(nv>mv){
                ans.push_back(other[0]-nv);
            }
            else{
                ans.push_back(other[1]-mv);
            }
        }
    }
    print(ans);
