...
int query2 (int root, int L, int R, int LL, int RR){
...
void compete_solution()
{
    ...
    for(int &val:queries){
        ...
        while(cur!=0){
            if(nums[cur-1]>val)
                break;
            int cur_high=get_high(val);
            int left=0,right=cur;
            
            while(left<right){
                int mid=(left+right)/2;
                //cout<<mid<<endl;
                int rst=query2(1,1,n,mid,cur-1);
                //cout<<"hi"<<endl;
                if(rst>=cur_high){
                    left=mid+1;
                }
            ...
            //cout<<left<<endl;
            int xor_rst=pre_xor[cur-1];
            if(left!=0){
                xor_rst^=pre_xor[left-1];
            }
            val^=xor_rst;
            cur=left;
            if(cur!=0){
